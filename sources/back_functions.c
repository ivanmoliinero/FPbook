...
void guardar_amistats(char *amistats, short n_elem, FILE *f)
{
    ...
    for (int dir = 0; dir < valors; dir++) // Files.
    {
        ...
        {
            if (amistats[dir] >= 0)
                fprintf(f, "%c", ' '); // En cas que el primer element sigui major o igual a 0, s'afegeix un espai més. Si no, aquesta posició l'ocuparà el signe negatiu.
...
bool data_compatible(char dia, char mes, short any)
{
    ...
    if (mes == 2 && dia == 29)                                                               // Comprovació cas any estacionari.
        ...
        correcte = false; // S'evita ocupar la pilla innecessàriament.
    else
    {
