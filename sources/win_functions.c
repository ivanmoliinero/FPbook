...
{
    main_screen_functionalities(&(dades_sis->win));
...
void main_screen_functionalities(finestra_t *win)
{
    ...
    g_signal_connect(win->finestra_principal.afegir_usuaris, "clicked", G_CALLBACK (afegir_usuaris_activate), (gpointer)win);
}
...
{
    finestra_t *win = ptr; // Es transforma el punter general en un finestra_t *.
    win->afegir_usuari.main_box = g_object_ref(win->afegir_usuari.main_box);
    gtk_container_remove(GTK_CONTAINER(win->main), win->afegir_usuari.main_box);
    show_main_window(win);
}
...
void afegir_usuaris_functionalities(info_t *dades_sis)
{
    ...
    g_signal_connect(win->afegir_usuari.go_back_button, "clicked", G_CALLBACK (afegir_usuaris_to_main_window), (gpointer)win);
